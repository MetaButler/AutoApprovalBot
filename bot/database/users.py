...
class UserChannelSettings(BASE):
    ...
    def __repr__(self):
        return (
            f"<UserChannelSettings user_id={self.user_id} channel_id={self.channel_id}>"
        )
...
class GroupSettings(BASE):
    ...
    def __init__(
        ...
        group_id: int,
        welcome_on: bool,
        welcome_message: str = ACCEPTED_TEXT,
...
async def update_user_channel_settings(user_id: int, channel_id: int) -> bool:
    try:
        async with async_session() as session:
            user_channel_settings, created = await get_or_create(
                session, UserChannelSettings, user_id=user_id, channel_id=channel_id
            )
            return created
...
async def delete_user_channel_settings(user_id: int, channel_id: int) -> bool:
    try:
        async with async_session() as session:
            user_channel_settings, _ = await get_or_create(
                session, UserChannelSettings, user_id=user_id, channel_id=channel_id
            )
...
async def set_welcome_message(group_id: int, new_message: str) -> bool:
    try:
        async with async_session() as session:
            group_settings, _ = await get_or_create(
                session, GroupSettings, welcome_on=True, group_id=group_id
            )
...
async def get_welcome_message(group_id: int) -> str:
    try:
        async with async_session() as session:
            group_settings, _ = await get_or_create(
                session, GroupSettings, group_id=group_id
            )
    ...
    try:
        async with async_session() as session:
            result = await session.execute(
                select(UserChannelSettings.user_id).filter_by(
                    channel_id=channel_or_group_id
                )
            )
        ...
        logger.error(
            f"Error fetching users in channel or group {channel_or_group_id}: {e}"
        )
        return []
